...
@Drone.on(events.NewMessage(incoming=True, pattern=r'/mychat(?:@\w+)?(?:\s|$)'))
async def my_chat(event):
    ...
    if target:
        ...
        await event.reply(
            f"**No custom transfer chat set.**\n\nContent is saved to the chat where you send the link.\n\n"
            f"Use `/setchat <chat_id>` to override this."
        )


@Drone.on(events.NewMessage(incoming=True, pattern=r'/clearchat(?:@\w+)?(?:\s|$)'))
